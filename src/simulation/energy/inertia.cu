...
namespace Inertia {
    ...
    template <typename Scalar>
    __global__ void gradHessianKern(const glm::tvec3<Scalar>* X, const glm::tvec3<Scalar>* XTilde, const Scalar* dev_mass, Scalar* gradient, Scalar* hessianVal, int* hessianRowIdx, int* hessianColIdx, int numVerts) {
        int idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx >= numVerts) {
            return;
        }
        int offset = idx * 3;
        glm::tvec3<Scalar> diff = dev_mass[idx] * (X[idx] - XTilde[idx]);
        gradient[offset] += diff.x;
        gradient[offset + 1] += diff.y;
        gradient[offset + 2] += diff.z;
        hessianVal[offset] = dev_mass[idx];
        hessianRowIdx[offset] = offset;
        hessianColIdx[offset] = offset;
        hessianVal[offset + 1] = dev_mass[idx];
...
template <typename Scalar>
void InertiaEnergy<Scalar>::GradientHessian(Scalar* grad, const SolverData<Scalar>& solverData, const SolverParams<Scalar>& solverParams, Scalar coef) const
{
    int threadsPerBlock = 256;
    int numBlocks = (solverData.numVerts + threadsPerBlock - 1) / threadsPerBlock;

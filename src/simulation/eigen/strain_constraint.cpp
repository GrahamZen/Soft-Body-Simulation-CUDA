...
void strain_constraint_t::project_wi_SiT_AiT_Bi_pi(q_type const& q, Eigen::VectorXd& b) const
{
    ...
    sigma(0) = clamp(sigma(0), sigma_min_, sigma_max_);
    sigma(1) = clamp(sigma(1), sigma_min_, sigma_max_);
    sigma(2) = clamp(sigma(2), sigma_min_, sigma_max_);

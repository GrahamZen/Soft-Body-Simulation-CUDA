...
__host__ __device__ dataType ccdTetrahedronIntersectionTest(const glmVec3& X0, const glmVec3& XTilt,
    ...
    glmVec3 tmpNor;
    float tmpT;
    tmpT = ccdTriangleIntersectionTest(X0, V0, x0, x1, x2, v0, v1, v2, XTilt, xTilt0, xTilt1, xTilt2, tmpNor);
    if (tmpT < t) {
        t = tmpT;
        nor = tmpNor;
    }
    t = glm::min(t, ccdTriangleIntersectionTest(X0, V0, x0, x1, x3, v0, v1, v3, XTilt, xTilt0, xTilt1, xTilt3, tmpNor));
    if (tmpT < t) {
        t = tmpT;
        nor = tmpNor;
    }
    t = glm::min(t, ccdTriangleIntersectionTest(X0, V0, x0, x2, x3, v0, v2, v3, XTilt, xTilt0, xTilt2, xTilt3, tmpNor));
    if (tmpT < t) {
        t = tmpT;
        nor = tmpNor;
    }
    t = glm::min(t, ccdTriangleIntersectionTest(X0, V0, x1, x2, x3, v1, v2, v3, XTilt, xTilt1, xTilt2, xTilt3, tmpNor));
    if (tmpT < t) {
        t = tmpT;
        nor = tmpNor;
    }
...
__host__ __device__ dataType tetrahedronTrajIntersectionTest(const GLuint* tets, const glmVec3& X0, const glmVec3& XTilt, const glm::vec3* Xs, const glm::vec3* XTilts, GLuint tetId, glm::vec3& nor) {
    ...
    return ccdTetrahedronIntersectionTest(X0, XTilt, x0, x1, x2, x3, xTilt0, xTilt1, xTilt2, xTilt3, nor);
}
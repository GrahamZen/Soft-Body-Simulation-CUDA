...
struct Intersection {
    float t = FLT_MAX;
    indexType faceIdx = (indexType)-1;
    __host__ __device__ bool operator<(const Intersection& other) const {
        return t < other.t;
    }
};
...
indexType raySimCtxIntersection(Ray r, int numTris, const indexType* Tri, const glm::tvec3<Scalar>* X) {
    Intersection intersection = thrust::transform_reduce(thrust::counting_iterator<indexType>(0),
        ...
        float t = rayTriangleIntersection(r, v0, v1, v2, true);
        return t > 0 ? Intersection{ t, idx } : Intersection{};
    },
        Intersection(),
        thrust::minimum<Intersection>());
    return intersection.faceIdx;
}

...
void pdSolver()
{
    ...
    std::vector<glm::vec3> vertices(number);
...
std::vector<GLuint> loadEleFile(const std::string& EleFilename)
{
    std::string line;
...
std::vector<glm::vec3> loadNodeFile(const std::string& nodeFilename) {
...
void init(const char* nodeFileName, const char* eleFileName)
{
    vertices = loadNodeFile(nodeFileName);
    idx = loadEleFile(eleFileName);
}
